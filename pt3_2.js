...
// Genera 4 negres a fila 3 en funciÃ³ del negres de les files 1 i 2 de manera
...
function generaNegresFila3(fila) {
  ...
  // ColÂ·loca els negres que falten evitant tres negres a la mateixa columna
  while (i <negresFila){
    var negre = Math.floor(Math.random()*casellesFila);
...
// FunciÃ³ per desordenar un array
...
function generarNumeros(){
  var col = 9;
  var final = 89;
  // Crea un array de 1-n en que cada elemente tÃ© el valor de la seva posiciÃ³
  var totalNumeros = [];
  for (let k=1;k<final+1;k++){
    totalNumeros.push(k);

  }
  // Divideix l'array en porcions, en aquest cas de 9 elements que sÃ³n els
  // elements per fila del cartrÃ³
  var start = 0;
  var end = col;
  for (var j=0;j<9;j++){
    // Divideix i desordena
    // Cada posiciÃ³ de grupsNumeros contÃ© la decena corresponent
...
function omplirNumeros () {
  // Per a cada columna del cartrÃ³ = grupsNumeros.length
  for (let i=0;i<grupsNumeros.length;i++){
    // Per a cada fila del cartrÃ³
    for (let j=0;j<cartro.length;j++){
      if (cartro[j][i]!=false){
        // Si el valor no Ã©s false, s'extreu del final de l'array corresponent
        // a la seva decena un nÃºmero del final de l'array que estÃ  desordenat
        // La funciÃ³ pop() extreu i elimina l'Ãºltim element d'un array
